...
const videoPosters = [img.hero, img.sectional, img.warm, img.teal];
const keepPlaying = (event: React.SyntheticEvent<HTMLVideoElement>) => {
...
export default function Home() {
  ...
  return <div className="site-shell" id="home">
    <header className={`site-nav ${scrolled ? "site-nav--scrolled" : ""}`}>
      ...
      <div className="nav-contact-links"><a href="tel:+919666687123">+91 96666 87123</a><a href="https://www.instagram.com/spr.homecrafts?igsi=a3NzanN4ZWVmczd2" target="_blank" rel="noreferrer">@spr.homecrafts</a></div>
    ...
    <main>
      <section className="hero-section hero-split-section" aria-labelledby="hero-title" onPointerDown={handleHeroPointerDown} onPointerUp={handleHeroPointerUp}>
        ...
        <aside className="hero-video-rail" aria-label="SPR Homecraft video gallery">
          ...
          <div className="hero-rail-grid">{heroSlides.map((slide, index) => <div className={`hero-rail-tile ${index === activeHero ? "active" : ""}`} key={slide.label}><video src={slide.video} poster={videoPosters[index]} muted autoPlay loop playsInline preload="auto" onCanPlay={keepPlaying} onLoadedData={keepPlaying} /><div className="hero-rail-tint" /><span className="hero-rail-index">0{index + 1}</span><span className="hero-rail-caption">{slide.label}</span></div>)}</div>
      ...
      <section className="intro-section section-pad reveal-section" data-reveal id="about">
      ...
      <section className="story-section section-pad reveal-section" data-reveal id="process">
        ...
        <div className="process-list">{[["01", "Understand", "We start with your room, your routines, your references."], ["02", "Design", "Proportion, comfort and material come together on paper."], ["03", "Craft", "Our makers bring the design to life with care and precision."], ["04", "Deliver", "Your piece arrives ready for the space it was made for."], ["05", "Install", "The final detail is placed, tuned and made at home."]].map(([no, title, copy]) => <div className="process-item reveal-row" data-reveal key={no}><span className="process-no">{no}</span><h3>{title}</h3><p>{copy}</p><ChevronRight size={18} /></div>)}</div>
      </section>

      <section className="services-section section-pad reveal-section" data-reveal id="furniture"><div className="section-heading-row"><div><p className="eyebrow">WHAT WE CREATE</p><h2>Rooms with<br /><em>something to say.</em></h2></div><p className="section-lede">Not off-the-shelf. Not one-size-fits-all. Just furniture shaped around a life in motion.</p></div><div className="service-grid">{services.map((service) => <article className="service-card reveal-card" data-reveal key={service.no}><div className="service-image"><img src={service.image} alt={service.name} /></div><div className="service-content"><span>{service.no}</span><div><h3>{service.name}</h3><p>{service.copy}</p></div><ArrowUpRight size={20} /></div></article>)}</div></section>

      <section className="featured-section section-pad reveal-section" data-reveal><div className="section-heading-row"><div><p className="eyebrow">THE COLLECTION</p><h2>The pieces<br /><em>that stay with you.</em></h2></div><button className="text-link" onClick={() => scrollTo("gallery")}>VIEW THE GALLERY <ArrowUpRight size={16} /></button></div><div className="feature-grid"><article className="feature-card feature-card--large reveal-card" data-reveal><img src={img.teal} alt="Teal custom sofa with gold detailing" /><div><span className="eyebrow">01 · CUSTOM SECTIONAL</span><h3>Modern L-Shape</h3><p>Comfort with a little more character.</p></div></article><article className="feature-card feature-card--small reveal-card" data-reveal><img src={img.grey} alt="Grey modular sectional sofa" /><div><span className="eyebrow">02 · FAMILY LOUNGE</span><h3>The Everyday</h3><p>Built for long conversations.</p></div></article></div></section>

      <section className="projects-section section-pad reveal-section" data-reveal id="projects"><div className="projects-top"><div><p className="eyebrow eyebrow--gold">SELECTED WORK</p><h2>Spaces we’ve<br /><em>crafted.</em></h2></div><div className="filter-row">{["All", "Residential", "Living rooms", "Custom furniture"].map((item) => <button className={filter === item ? "active" : ""} key={item} onClick={() => setFilter(item)}>{item}</button>)}</div></div><div className="project-masonry">{filteredProjects.map((project) => <article className={`project-card project-card--${project.span} reveal-card`} data-reveal key={project.title}><img src={project.image} alt={project.title} /><div className="project-overlay"><span>{project.type}</span><h3>{project.title}</h3><ArrowUpRight size={20} /></div></article>)}</div></section>

      <section className="reel-section section-pad reveal-section" data-reveal id="gallery"><div className="reel-heading"><p className="eyebrow">A CLOSER LOOK</p><h2>See it<br /><em>come together.</em></h2><p>Real spaces. Real materials. A glimpse into the details that make a piece feel like yours.</p></div><div className="video-reel">{videos.map((video, index) => <div className="video-tile reveal-card" data-reveal key={video}><video src={video} poster={videoPosters[index]} muted loop autoPlay playsInline preload="auto" onCanPlay={keepPlaying} onLoadedData={keepPlaying} /><div className="video-top"><span>0{index + 1}</span><span>SPR / REEL</span></div><span className="play-mark"><Play size={14} fill="currentColor" /></span></div>)}</div></section>

      <section className="contact-section reveal-section" data-reveal><div className="contact-mark"><img src={logo} alt="SPR Homecraft" /></div><div><p className="eyebrow eyebrow--gold">YOUR SPACE, NEXT</p><h2>Let’s make<br /><em>room for more.</em></h2><p>Tell us what you’re imagining. We’ll help you find the shape, material and finish to make it real.</p><button className="button button--gold" onClick={openQuote}>BOOK A CONSULTATION <ArrowUpRight size={17} /></button><div className="contact-links"><a href="tel:+919666687123"><Phone size={15} /> +91 96666 87123</a><a href="https://www.instagram.com/spr.homecrafts?igsi=a3NzanN4ZWVmczd2" target="_blank" rel="noreferrer"><Instagram size={15} /> @spr.homecrafts</a></div><div className="contact-locations"><p className="eyebrow eyebrow--gold">VISIT THE WORKSHOP</p><a href="https://www.google.com/maps/place/Spr+homecraft+unit+1,+Sai+Nagar+Colony,+Badnaipet,+Hyderabad,+Telangana+500005/@17.3070702,78.5128135,15z/data=!4m6!3m5!1s0x3bcba3003a0da651:0xa80f2e48a0cde813!8m2!3d17.3070702!4d78.5128135!16s%2Fg%2F11z1k8l_2j?g_ep=Eg1tbF8yMDI2MDgxN18wIOC7DCoASAJQAg%3D%3D" target="_blank" rel="noreferrer">Unit 1 · Sai Nagar Colony, Badnaipet <ArrowUpRight size={14} /></a><a href="https://www.google.com/maps/place/7G4Q%2BPJG+Spr+homecraft+unit+2,+Nadargul,+Telangana+501510/@17.2565603,78.5391526,16z/data=!4m6!3m5!1s0x3bcba3007f48dea9:0x4b8049e326a59da2!8m2!3d17.2565603!4d78.5391526!16s%2Fg%2F11nq1ggj33?g_ep=Eg1tbF8yMDI2MDgxN18wIOC7DCoASAJQAg%3D%3D" target="_blank" rel="noreferrer">Unit 2 · Nadargul, Telangana <ArrowUpRight size={14} /></a></div></div></section>
    </main>
